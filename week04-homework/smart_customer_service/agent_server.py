...
from smart_customer_service.plugin_loader import load_plugins
...
def _build_agent(force_rebuild: bool = False) -> Any:
    ...
    system = (
        "你是订单客服助手。当前日期（今天）是 {today}，昨天是 {yesterday}。\n"
        "当用户提到「我昨天下的单」「昨天买的」等相对时间时，你必须先和用户确认具体日期："
        "在回复中明确写出「今天」和「昨天」对应的具体日期（今天：{today}，昨天：{yesterday}），"
        "再引导用户提供订单号等后续操作。\n"
