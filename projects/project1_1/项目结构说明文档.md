...
### 1.2 主要功能
- **智能对话**：基于 DeepSeek 大语言模型的自然语言理解和生成（兼容 OpenAI API 格式）
...
- **LangChain-OpenAI (>=0.1.7)**: OpenAI 兼容模型集成（实际使用 DeepSeek API）
- **LangChain-Community (>=0.0.38)**: 社区工具和集成
- **LangChain-Core (>=0.1.52)**: LangChain 核心组件

#### AI 和语言模型
- **DeepSeek API**: 使用 DeepSeek 大语言模型（兼容 OpenAI API 格式，通过 base_url 配置）
- **注意**: 虽然使用 LangChain-OpenAI 包，但实际配置为调用 DeepSeek API
...
#### 必需的外部服务
- **DeepSeek API**: 需要有效的 DeepSeek API 密钥（兼容 OpenAI API 格式）
...
# DEEPSEEK_API_BASE=https://api.deepseek.com/v1  # 可选，默认使用官方地址
#
# 高德地图 API 配置（必需）
...
**问题**: `openai.error.AuthenticationError: Invalid API key` 或连接错误
**解决方案**:
...
```bash
# DeepSeek API 配置（必需）
# 注意：虽然代码中使用 LangChain-OpenAI，但实际调用的是 DeepSeek API
DEEPSEEK_API_KEY=your_deepseek_api_key_here
DEEPSEEK_API_BASE=https://api.deepseek.com/v1  # 可选，默认使用官方地址

# 向后兼容：如果设置了 OPENAI_API_KEY，也会被使用（优先使用 DEEPSEEK_API_KEY）
# OPENAI_API_KEY=your_openai_api_key_here  # 可选，用于向后兼容
# OPENAI_API_BASE=https://api.openai.com/v1  # 可选，用于向后兼容
...
### C. API 密钥获取指南
1. **DeepSeek API**:
   - 访问 https://platform.deepseek.com/ 或 https://www.deepseek.com/
   - 注册账号并完成验证
   - 在 API Keys 页面创建新密钥
   - 注意：DeepSeek API 兼容 OpenAI API 格式，可以直接使用 LangChain-OpenAI 包
