...
- **openai (>=1.3.0)**: OpenAI API 客户端（用于调用 DeepSeek API，因为 DeepSeek API 兼容 OpenAI API 格式）
...
#### 必需的外部服务
- **DeepSeek API**: 需要有效的 DeepSeek API 密钥（必需，用于智能体对话）
  - 注意：DeepSeek API 兼容 OpenAI API 格式，可以使用 `openai` 包调用
...
# 编辑 .env 文件，填入必要的配置
# 特别是 DEEPSEEK_API_KEY（必需，用于智能体对话）
# 或者 OPENAI_API_KEY（可选，用于向后兼容）
```
...
4. 检查网络连接和防火墙设置
5. 如果使用代理，确保代理配置正确
6. 向后兼容：如果设置了 `OPENAI_API_KEY`，也会被使用（优先使用 `DEEPSEEK_API_KEY`）
...
```bash
# DeepSeek API 配置（必需）
# 注意：虽然使用 openai 包，但实际调用的是 DeepSeek API
...
# OPENAI_MODEL=deepseek-chat  # 已改为 DeepSeek
# OPENAI_BASE_URL=https://api.deepseek.com/v1  # 已改为 DeepSeek
